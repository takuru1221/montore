...
const sounds = {
  training: new Audio("sounds/training.mp3"),
  protein: new Audio("sounds/protein.mp3"),
...
function setupEvents() {
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      const ok = confirm("本当にデータをリセットしますか？");
      if (!ok) return;
...
function updateHome() {
  ...
  document.getElementById("homeMonsterName").textContent = getMonsterDisplayName();
  ...
  document.getElementById("homeBenchText").textContent =
    state.maxWeights?.bench ? `${state.maxWeights.bench}kg` : "未記録";

  document.getElementById("homeSquatText").textContent =
    state.maxWeights?.squat ? `${state.maxWeights.squat}kg` : "未記録";

  document.getElementById("homeDeadliftText").textContent =
    state.maxWeights?.deadlift ? `${state.maxWeights.deadlift}kg` : "未記録";

  document.getElementById("weeklyTrainingText").textContent = getWeeklyTrainingCount();
  document.getElementById("totalTrainingText").textContent = state.totalTraining;

  const latest = state.weightHistory[state.weightHistory.length - 1];

  document.getElementById("homeCurrentWeightText").textContent =
    latest ? `${latest.weight}kg` : "未記録";

  document.getElementById("homeTargetWeightText").textContent =
    state.targetWeight ? `${state.targetWeight}kg` : "未設定";

  document.getElementById("homeTargetDateText").textContent =
    state.targetDate || "未設定";

  document.getElementById("daysLeftText").textContent = getDaysLeftText();

  const needText = getNeedPerDayText();
  document.getElementById("homeNeedPerDayText").textContent =
    needText === "未計算" ? "目標を設定すると表示されます" : `1日あたり ${needText}`;
}

function updateTrain() {
  document.getElementById("levelText").textContent = state.level;
  document.getElementById("homeTotalPowerText").textContent = getTotalPower();

  const expPercent = Math.min(100, Math.floor((state.exp / getNeedExp()) * 100));
  document.getElementById("expFill").style.width = `${expPercent}%`;
  document.getElementById("homeExpText").textContent = `${state.exp} / ${getNeedExp()}`;

  document.getElementById("nextEvolutionText").textContent = getNextEvolutionText();
  ...
  document.getElementById("todayTrainingText").textContent = state.today.training;
  document.getElementById("todayProteinText").textContent = state.today.protein;
  document.getElementById("todaySleepText").textContent = state.today.sleep;
...
function updateMuteButton() {
  ...
  muteBtn.textContent = state.muted ? "🔇 効果音 OFF" : "🔊 効果音 ON";
}
